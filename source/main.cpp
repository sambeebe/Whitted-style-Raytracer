...
bool shadowFeeler(vec4 p0, Object *object){
    ...
    bool inShadow = false;
    vec4 L = normalize(lightPosition - p0);


    for(int i=0; i <sceneObjects.size();i++){
        Object::IntersectionValues intersect = (sceneObjects[i]->intersect(p0, L));
        if( intersect.t_w != std::numeric_limits<double>::infinity() && intersect.t_w<length(L)){
            inShadow = true;
        }
    }
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
    ...
    Object::IntersectionValues min_intersect = sceneObjects[0]->intersect(p0,E);
    ...
    if(min_intersect.t_w != std::numeric_limits<double>::infinity() ){
        ...
        color4 material_ambient(sceneObjects[id]->shadingValues.color.x*sceneObjects[id]->shadingValues.Ka,
                                sceneObjects[id]->shadingValues.color.y*sceneObjects[id]->shadingValues.Ka,
                                sceneObjects[id]->shadingValues.color.z*sceneObjects[id]->shadingValues.Ka, 1.0 );

        color4 material_diffuse(sceneObjects[id]->shadingValues.color.x,
                                sceneObjects[id]->shadingValues.color.y,
                                sceneObjects[id]->shadingValues.color.z, 1.0 );

        color4 material_specular(sceneObjects[id]->shadingValues.Ks,
                                 sceneObjects[id]->shadingValues.Ks,
                                 sceneObjects[id]->shadingValues.Ks, 1.0 );
        ...
        vec4 N = normalize(min_intersect.N_w);
        vec4 V = normalize(E - min_intersect.P_w);
        vec4 R = normalize(reflect(E, N));
        ...
        float Ks = sceneObjects[id]->shadingValues.Ks;
        vec4 Kd = sceneObjects[id]->shadingValues.Kd;
        float Ka = sceneObjects[id]->shadingValues.Ka;
        float Kt = sceneObjects[id]->shadingValues.Kt;
        
        if (Ka > 0){
            ambient = Ka * ambient_product;
        }
        if (Kd > 0) {
            diffuse = Kd * diffuse_product * dot(N, L);
        }
        if (Ks > 0){
            
            specular =  Ks * specular_product * castRay(min_intersect.P_w, R, sceneObjects[id], depth+1);
        }
        ...
        if (Kt > 0) {
            ...
            {
                n1 = 1.0;
                n2 =  sceneObjects[id]->shadingValues.Kr;
            } else
            {
                n1 =  sceneObjects[id]->shadingValues.Kr;
                n2 = 1.0;
            }
            float n12 = n1 / n2;
            float cos_i = -dot(N,I);
            ...
            float cos_t = cos( asin((n1 / n2) * sin(  acos( dot(I,N)))) );


            float Rperp =  ((n1 * cos_i) - (n2 * cos_t)) / ((n1 * cos_i) + (n2 * cos_t));
            Rperp = (Rperp * Rperp);
            float Rpara =  ((n2 * cos_i ) - (n1 * cos_t)) / ((n2 * cos_i) + (n1 * cos_t));
            Rpara = (Rpara * Rpara);
     
            float insqrt = (sqrt(1 - (n12 * n12) * (1 - ((dot(N, I)*dot(N, I))))));
//            float dt = dot(I,N);
//            float discriminant = 1.0 - n12 *n12*(1-dt*dt);
//            vec4 T = n12*(I - N*dt) - N * sqrt(discriminant);
            vec4 T = N * (-n12*dot(N,I) - sqrt(1 - n12*n12*(1.0 - dot(N,I)*dot(N,I)))) + I * n12;
            T.w = 0.0;
            T = normalize(T);
            

            //
            float Reflect = (Rperp+Rpara)/2;;
            float Refract = 1.0 - Reflect;

            
            vec4 refrCol, reflCol;
            if (insqrt > EPSILON){
            refrCol = Kt*Refract*castRay(min_intersect.P_w, T, sceneObjects[id], depth+1);
            reflCol = Reflect*castRay(min_intersect.P_w, R,sceneObjects[id], depth+1);
            }
            
            transmitted = refrCol+reflCol;
        }
        color= ambient+diffuse+specular+transmitted;
        
        if(shadowFeeler (min_intersect.P_w, sceneObjects[id]) != 0){
...
void rayTrace(){
    ...
    write_image("output", buffer, GLState::window_width, GLState::window_height, 4);
  ...
  unsigned char *buffer = new unsigned char[GLState::window_width*GLState::window_height*4];
   ...
   for(unsigned int i=0; i < GLState::window_width; i++){
    for(unsigned int j=0; j < GLState::window_height; j++){
        ...
        float uOffset = (float) i * 4;
        float vOffset = (float) j * 4;
        float nxCells = (float) GLState::window_width * 4;
        float nyCells = (float) GLState::window_height * 4;
        vec4 color = vec4 (0., 0., 0., 0.);
        ...
        for (int dj = 0; dj < 4; dj++){
            for (int di = 0; di < 4; di++){
                ...
                float randf = dis(gen);
                std::cout << randf << "\n";
                float u = float(uOffset + di) + randf / nxCells;
                float v = float(nyCells - vOffset) + randf / nyCells;
                std::vector < vec4 > ray_o_dir = findRay(v,u);
                color += castRay(ray_o_dir[0], vec4(ray_o_dir[1].x, ray_o_dir[1].y, ray_o_dir[1].z, 0.0), NULL, 0);
      ...
      int idx = j*GLState::window_width+i;
        color /= 16.;
//      color = vec4(sqrt(color.x),sqrt(color.y),sqrt(color.z), sqrt(color.w) );
...
//      sceneObjects[sceneObjects.size()-1]->setModelView( );
  }
    
    {
        sceneObjects.push_back(new Sphere("Diffuse sphere2"));
        Object::ShadingValues _shadingValues;
        ...
        _shadingValues.Kr = 0.0;
        sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
        sceneObjects[sceneObjects.size()-1]->setModelView( (   Translate(1.0, 0., 0.) *Scale(0.75, 0.75, 0.75))    ) ;
        //      sceneObjects[sceneObjects.size()-1]->setModelView( );
    }
